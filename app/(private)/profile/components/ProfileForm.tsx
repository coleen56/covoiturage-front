...
export default function ProfileForm({user} : Readonly<{ user: User }>) {
    ...
    return (
        <>
            {'error' in (userState ?? {}) && (
                <ErrorAlert message={(userState as { error: string }).error} />
            )}
            {'success' in (userState ?? {}) && (
                <SuccessAlert message={(userState as unknown as { success: string }).success} />
            )}
            ...
            <form className="w-100 space-y-3 mb-5" onSubmit={handleCarSubmit}>
                {'error' in (carState ?? {}) && (
                    <ErrorAlert message={(carState as { error: string }).error} />
                )}
                {'success' in (carState ?? {}) && (
                    <SuccessAlert message={(carState as unknown as { success: string }).success} />
                )}
